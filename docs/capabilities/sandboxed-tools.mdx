...
IronClaw maintains a persistent per-user permission table that controls whether individual tools require approval on each invocation. This replaces one-time "allow this tool?" prompts with durable settings.
...
| Level | Behavior |
|-------|----------|
| `always_allow` | Tool runs without any approval prompt |
| `ask_each_time` | Tool requires approval on every invocation (default for dangerous tools) |
| `disabled` | Tool is blocked — attempts to use it fail immediately |
