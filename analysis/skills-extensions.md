...
- **WASM channels** — WebAssembly modules that act as input channels (Telegram, Slack, etc.); support hot-activation at runtime without a restart
...
| `WasmChannel` | In-process WASM sandbox | Capabilities file | Hot-activation supported (no restart needed) |
...
- WASM channel: loads channel from `~/.ironclaw/channels/<name>.wasm`, injects host-side credentials (secrets resolved from the secrets store are substituted into URL and header placeholders at the host boundary — the WASM module never receives plaintext values), registers webhook endpoint via `WasmChannelRouter`, calls `channel_manager.hot_add()` to start receiving messages without a restart. If the channel is already active, `refresh_active_channel()` re-injects credentials and refreshes the webhook secret instead. On first activation, the channel name is persisted to the `activated_channels` settings key so the channel auto-activates on the next restart.

**Setup secrets (WASM channels and tools)**: `ExtensionManager::save_setup_secrets()` collects a map of `{secret_name → value}` from the web UI setup form (or the `ironclaw tool setup` CLI command), stores them in the `SecretsStore` via `create()`, and then attempts `activate_wasm_channel()`. Returns a `SetupResult` with the activation status and a human-readable message. Auto-generated secrets (declared with `auto_generate` in the capabilities file) are generated by the host rather than prompted.

**Duplicate activation prevention**: At startup, `set_active_channels()` records which WASM channels were loaded. `activate_wasm_channel()` checks `active_channel_names` before attempting a new load — if the channel is already present it calls `refresh_active_channel()` instead of creating a second instance. This prevents duplicate registrations when the UI re-triggers activation.
...
- WASM channel: hot-deactivation not currently supported; requires deleting files from `~/.ironclaw/channels/` and restarting

### 3.4 WASM Tool Setup Schema and Display Names (v0.13.0, PR #437)

**Source:** `src/tools/wasm/capabilities_schema.rs`, `src/extensions/mod.rs`, `src/extensions/manager.rs`

**WASM tool setup schema:** The `CapabilitiesFile` for WASM tools now supports an optional `setup` key that mirrors the channel `setup.required_secrets` pattern:
...
`ToolSetupSchema.required_secrets` is a list of `ToolSecretSetupSchema` items, each with a `name` (key in the secrets store), a `prompt` (user-facing label in the setup modal), and an optional `optional: bool` flag.

`ExtensionManager::get_setup_schema()` and `save_setup_secrets()` both now handle `WasmTool` kind in addition to `WasmChannel`. After secrets are saved, `save_setup_secrets()` auto-activates the tool (loading the WASM binary and registering its tools), so users do not need a separate activation step.

**Display name labels:** `InstalledExtension` now includes a `display_name: Option<String>` field. When populated (e.g. `"Telegram Channel"`, `"Telegram Tool"`, `"GitHub MCP"`), the web UI extension cards display it instead of the raw `name` key. The extension registry entries for Telegram, Slack, Discord, and WhatsApp have been updated with distinct display names to distinguish the channel variant from the tool variant.

The button label in the extension card setup modal changes based on secret configuration state:
- No secrets configured: `"Setup"` button.
- At least one secret present: `"Reconfigure"` button.

### 3.5 Built-in Extension Registry
...
### 3.6 Embedded Registry Catalog (v0.10.0)
...
### 3.7 WASM Bundle Install Pipeline (v0.10.0)
...
### 3.8 Online Discovery (`discovery.rs`)
