...
| `worker` | `src/worker/` | Runs inside Docker containers: `Worker` execution loop, tool calls via LLM reasoning, Claude Code bridge (spawns `claude` CLI), orchestrator HTTP client, proxy LLM provider that forwards requests through orchestrator. The `ironclaw-worker` Docker image is published to Docker Hub and can be built locally via the setup wizard (`ironclaw onboard`). Added v0.25.0 (PR #1757, #1979). |
