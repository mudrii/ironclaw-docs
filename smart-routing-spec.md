...
**Status:** Implemented  
**Original author:** Microwave  
**Original spec date:** 2026-02-19
...
1. **RoutingProvider** — wrapper implementing `LlmProvider` trait
2. **Scorer** — pure function, no I/O, fast (~1ms)
3. **Config schema** — `LlmConfig` + `SmartRoutingConfig`
4. **Telemetry** — routing decisions logged for observability
...
3. [x] Extend config schema (`src/config/llm.rs`)
